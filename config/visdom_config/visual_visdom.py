...
def visual_image_sup(vis, mask_train, pred_train, mask_val, pred_val):
    ...
    vis.heatmap(pred_train, win='train_pred', opts=dict(title='Train Pred', colormap='Viridis'))
    vis.heatmap(mask_val, win='val_mask', opts=dict(title='Val Mask', colormap='Viridis'))
    vis.heatmap(pred_val, win='val_pred', opts=dict(title='Val Pred', colormap='Viridis'))
