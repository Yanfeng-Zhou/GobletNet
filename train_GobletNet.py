...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_dataset', default='.../GobletNet/dataset/EPFL')
    ...
    parser.add_argument('--input2', default='H_0.2', type=str)
    ...
    parser.add_argument('-n', '--network', default='GobletNet', type=str)
    ...
    path_trained_models = path_trained_models + '/' + str(args.network) + '-l=' + str(args.lr) + '-e=' + str(args.num_epochs) + '-s=' + str(args.step_size) + '-g=' + str(args.gamma) + '-b=' + str(args.batch_size) + '-w=' + str(args.warm_up_duration)+'-'+str(args.input1)+'-'+str(args.input2)
    if not os.path.exists(path_trained_models) and rank == args.rank_index:
        os.mkdir(path_trained_models)
    ...
    path_seg_results = path_seg_results + '/' + str(args.network) + '-l=' + str(args.lr) + '-e=' + str(args.num_epochs) + '-s=' + str(args.step_size) + '-g=' + str(args.gamma) + '-b=' + str(args.batch_size) + '-w=' + str(args.warm_up_duration)+'-'+str(args.input1)+'-'+str(args.input2)
    if not os.path.exists(path_seg_results) and rank == args.rank_index:
        os.mkdir(path_seg_results)

    if args.vis and rank == args.rank_index:
        visdom_env = str(str(os.path.split(args.path_dataset)[1]) + '-' + args.network + '-l=' + str(args.lr) + '-e=' + str(args.num_epochs) + '-s=' + str(args.step_size) + '-g=' + str(args.gamma) + '-b=' + str(args.batch_size) + '-w=' + str(args.warm_up_duration)+'-'+str(args.input1)+'-'+str(args.input2))
        ...
        if count_iter % args.display_iter == 0:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(dataloaders['val']):
                    ...
                    optimizer.zero_grad()
                    outputs_val = model(inputs_val, inputs_val_wavelet)
                    torch.cuda.empty_cache()
